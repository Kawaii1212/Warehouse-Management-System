...
public class PromotionController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if ("create".equals(action)) {
                ...
                System.out.println("Unknown action: " + action + ", redirecting to Promotion page");
                response.sendRedirect("Promotion?error=unknown_action");
            }
        } catch (Exception e) {
            System.out.println("❌ [PromotionController] Unexpected error in doPost: " + e.getMessage());
            e.printStackTrace();
            response.sendRedirect("Promotion?error=server_error");
        }
    }
    ...
    private void handleUpdate(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idParam == null || idParam.trim().isEmpty()) {
            ...
            response.sendRedirect("Promotion?error=invalid_id&message=" + java.net.URLEncoder.encode("Mã khuyến mãi không được để trống", "UTF-8"));
            return;
        }
        ...
        try {
            ...
            Promotion existingPromo = promotionDAO.getPromotionById(promotionId);
            ...
            if (updated) {
                ...
                System.out.println("🗑️ Deleting existing branches and products...");
                ...
                if (branchIds != null && branchIds.length > 0) {
                    ...
                    if (!branchesInserted) {
                        System.out.println("⚠️ Warning: Failed to insert some branches");
                    }
                } else {
                    System.out.println("ℹ️ No branches to insert");
                }
                ...
                if (productDetailIds != null && productDetailIds.length > 0) {
                    ...
                    if (!productsInserted) {
                        System.out.println("⚠️ Warning: Failed to insert some products");
                    }
                } else {
                    System.out.println("ℹ️ No products to insert");
                }
                
                System.out.println("✅ Update successful, redirecting...");
                response.sendRedirect("Promotion?success=update");
            } else {
                System.out.println("❌ Update failed - no rows affected. Promotion ID might not exist or data unchanged.");
                response.sendRedirect("Promotion?error=update_failed&message=" + java.net.URLEncoder.encode("Không thể cập nhật khuyến mãi. Vui lòng kiểm tra lại dữ liệu.", "UTF-8"));
            }
        } catch (ParseException e) {
            System.out.println("❌ ParseException: " + e.getMessage());
            e.printStackTrace();
            response.sendRedirect("Promotion?error=invalid_date_format&message=" + java.net.URLEncoder.encode("Định dạng ngày không hợp lệ: " + e.getMessage(), "UTF-8"));
        } catch (NumberFormatException e) {
            System.out.println("❌ NumberFormatException: " + e.getMessage());
            e.printStackTrace();
            response.sendRedirect("Promotion?error=invalid_id&message=" + java.net.URLEncoder.encode("Mã khuyến mãi không hợp lệ: " + e.getMessage(), "UTF-8"));
        } catch (IllegalArgumentException e) {
            System.out.println("❌ IllegalArgumentException: " + e.getMessage());
            e.printStackTrace();
            response.sendRedirect("Promotion?error=invalid_data&message=" + java.net.URLEncoder.encode("Dữ liệu không hợp lệ: " + e.getMessage(), "UTF-8"));
        } catch (Exception e) {
            System.out.println("❌ Unexpected exception: " + e.getMessage());
            e.printStackTrace();
            response.sendRedirect("Promotion?error=update_failed&message=" + java.net.URLEncoder.encode("Lỗi hệ thống: " + e.getMessage(), "UTF-8"));
        }
    }
