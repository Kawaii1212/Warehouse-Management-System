...
public class WHExportOrdersController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Kiểm tra quyền truy cập: chỉ role 3 (Warehouse Manager) mới có thể truy cập
            int roleId = currentUser.getRoleId();
            if (roleId != 3) {
                // Nếu không phải role 3, chuyển hướng về trang chủ với thông báo lỗi
                response.sendRedirect(request.getContextPath() + "/WareHouseProduct?error=access_denied&message=" 
                    + java.net.URLEncoder.encode("Bạn không có quyền truy cập chức năng này. Chỉ quản lý kho (role 3) mới có thể truy cập trang Xuất hàng.", "UTF-8"));
                return;
            }
            ...
            if (warehouseId != null) {
                ...
                System.out.println("Found " + exportOrders.size() + " export orders for warehouse " + warehouseId);
                System.out.println("Total count: " + totalCount);
                
                // If no results found with warehouseId filter, try showing all (fallback)
                if (totalCount == 0) {
                    System.out.println("=== [WHExportOrdersController] No results with warehouseId filter, trying all warehouses ===");
                    exportOrders = requestDAO.getExportRequestsWithFilterNoWarehouse(
                            fromDate, toDate, productId, status, page, pageSize);
                    totalCount = requestDAO.getExportRequestsCountNoWarehouse(fromDate, toDate, productId, status);
                    System.out.println("Found " + exportOrders.size() + " export orders (all warehouses - fallback)");
                    System.out.println("Total count: " + totalCount);
                }
            } else {
                // If no warehouseId, show all export orders (for admin or users without warehouse assignment)
                System.out.println("=== [WHExportOrdersController] No warehouseId, showing all export orders ===");
                System.out.println("FromDate: " + fromDate);
                System.out.println("ToDate: " + toDate);
                System.out.println("ProductID: " + productId);
                System.out.println("Status: " + status);
                ...
                System.out.println("Found " + exportOrders.size() + " export orders (all warehouses)");
                System.out.println("Total count: " + totalCount);
            }
