...
//Được sử dụng khi export tạo ra một MovementRequest
    ...
    public List<StockMovementsRequest> getExportRequestsWithFilter(
        ...
        sql.append("""
            SELECT 
                ...
                u.FullName AS CreatedByName,
                smr.Note,
                COALESCE(smrsp.ResponseStatus, N'Chờ xử lý') AS ResponseStatus
            FROM StockMovementsRequest smr
        ...
        if (status != null && !status.isBlank() && !status.equals("Tất cả")) {
            sql.append(" HAVING COALESCE(smrsp.ResponseStatus, N'Chờ xử lý') = ?");
            params.add(status);
        }
    ...
    public int getExportRequestsCount(int warehouseId, String fromDate, String toDate,
        ...
        if (status != null && !status.isBlank() && !status.equals("Tất cả")) {
            sql.append(" AND COALESCE(smrsp.ResponseStatus, N'Chờ xử lý') = ?");
            params.add(status);
        }
    ...
    /**
     * Lấy danh sách đơn xuất hàng với bộ lọc (không filter theo warehouseId - cho admin hoặc khi user không có warehouseId)
     */
    public List<StockMovementsRequest> getExportRequestsWithFilterNoWarehouse(
    ...
    /**
     * Đếm tổng số đơn xuất (không filter theo warehouseId)
     */
    public int getExportRequestsCountNoWarehouse(String fromDate, String toDate,
            String productId, String status) {
        StringBuilder sql = new StringBuilder();
        sql.append("""
            SELECT COUNT(DISTINCT smr.MovementID)
